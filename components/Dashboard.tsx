...
const HeroHeader = ({ language }: { language: string }) => {
  ...
  return (
    <div className="mb-6 w-full text-center">
      <h1 className="text-white font-[1000] text-5xl md:text-7xl uppercase italic tracking-tighter leading-none inline-block">
        {t.dash_welcome} <span className="text-[#EE2523]">25/26</span>
      </h1>
    </div>
  );
};

// Estilo para cajones estándar
...
const Dashboard: React.FC<{ onNavigate?: (view: ViewType) => void, language?: string }> = ({ onNavigate = (_v: ViewType) => {}, language = 'ES' }) => {
  const t = translations[language] || translations['ES'];
      ...
      <div className="grid grid-cols-1 gap-8 w-full">
        ...
        <section className="w-full">
          <div className="mb-4 flex items-center gap-6">
            <h3 className="text-white/30 font-black text-[10px] uppercase tracking-[0.4em] shrink-0">{t.dash_news_title}</h3>
            <div className="h-px flex-1 bg-white/5"></div>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full">
            <NewsCard 
              title={language === 'EU' ? "BERRIKUSPEN MEDIKUAK" : "REVISIONES MÉDICAS"} 
              category={t.nav_medical} 
              badgeStyle="bg-blue-900/20 text-blue-400"
              iconColor="text-sky-400"
              description={language === 'EU' ? "Lezamako talde filialei begira programazio aktiboa." : "Programación activa para los equipos filiales en Lezama."}
            ...
            />
            <NewsCard 
              title={t.nav_evaluation} 
              category={t.nav_methodology} 
              badgeStyle="bg-yellow-900/20 text-yellow-500"
              iconColor="text-amber-400"
              description={language === 'EU' ? "Errendimendu eta potentzial neurri berriak gaituta." : "Nuevas métricas de rendimiento y potencial habilitadas."}
              button={t.dash_btn_eval}
            ...
            />
            <NewsCard 
              title={language === 'EU' ? "TALDEAREN ARROPA BANAKETA" : "ENTREGA ROPA DEL CLUB"} 
              category="LOGÍSTICA" 
              badgeStyle="bg-red-900/20 text-red-500"
              iconColor="text-rose-500"
              description={language === 'EU' ? "25/26 denboraldiko tallaje ofizialaren prozesua irekita." : "Proceso abierto de tallaje oficial temporada 25/26."}
              button={t.dash_btn_form}
        ...
        <section className="w-full">
          <div className="mb-4 flex items-center gap-6">
            <h3 className="text-white/30 font-black text-[10px] uppercase tracking-[0.4em] shrink-0">{t.dash_pillars}</h3>
            <div className="h-px flex-1 bg-white/5"></div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full">
            <PillarCard id="EQUIPO" label={t.dash_pillar_team} sub={t.dash_pillar_team_sub} iconColor="text-cyan-400" onNavigate={onNavigate} icon={<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857"/></svg>} />
            <PillarCard id="PLANTILLAS" label={t.dash_pillar_player} sub={t.dash_pillar_player_sub} iconColor="text-rose-400" onNavigate={onNavigate} icon={<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14"/></svg>} />
            <PillarCard id="ENTRENADOR" label={t.dash_pillar_coach} sub={t.dash_pillar_coach_sub} iconColor="text-amber-400" onNavigate={onNavigate} icon={<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7"/></svg>} />
          </div>
        </section>
        ...
        <section className="w-full">
          <div className="mb-4 flex items-center gap-6">
            <h3 className="text-white/30 font-black text-[10px] uppercase tracking-[0.4em] shrink-0">{t.dash_method}</h3>
            <div className="h-px flex-1 bg-white/5"></div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 w-full">
            <ActionAccessCard id="TAREAS" label={t.dash_action_tasks} sub={t.dash_action_tasks_sub} iconColor="text-emerald-400" onNavigate={onNavigate} icon={<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>} />
            <ActionAccessCard id="SESIONES" label={t.dash_action_sessions} sub={t.nav_soon} iconColor="text-amber-500" onNavigate={onNavigate} icon={<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7"/></svg>} />
            <ActionAccessCard id="PARTIDOS" label={t.dash_action_matches} sub={t.dash_action_matches_sub} iconColor="text-orange-500" onNavigate={onNavigate} icon={<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7"/></svg>} />
            <ActionAccessCard id="VIDEOTECA" label={t.dash_action_videoteca} sub={t.dash_action_videoteca_sub} iconColor="text-indigo-400" onNavigate={onNavigate} icon={<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14"/></svg>} />
          </div>
        </section>
